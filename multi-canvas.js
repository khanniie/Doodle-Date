...
var deleteLayer = function(aCanvas) {
    if (aCanvas == "canvas0") { alert("You can't delete the background! Sorry."); } else {
...
$("#mergeLayerButton").click(function() {
    if (currentCanvas == "canvas0") { alert("You can't merge down! There's nothing below! Sorry."); } else {
        var n = currentCanvas.slice(currentCanvas.length - 1);
        var deleted2 = document.getElementById("preview" + n);
        var a = 1;
        selectedLayer = "preview" + (n - a);
...
cover.addEventListener('mousemove', function(event) {
    if (mouseIsDown) {
        ...
        if (PorE == "eraser") {
            ...
            var drawing = new Drawing(mousePosition.x, mousePosition.y, event.offsetX, event.offsetY, lineCol, lineWid, lineOpacity, lineSoftness, "pen", currentCanvas); }
...
function _drag_init(elem) { selected = elem;
